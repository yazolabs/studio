...
export async function listAccountsPayable(params?: AccountsPayableQueryParams) {
  const { data } = await api.get<CollectionResponse<AccountPayable>>(basePath, {
    params,
  });
  return unwrap<AccountPayable[]>(data as any);
}

export async function getAccountPayable(id: number) {
  const { data } = await api.get<ResourceResponse<AccountPayable>>(
    `${basePath}/${id}`
  );
  return unwrap<AccountPayable>(data as any);
}
...
export async function createAccountPayable(payload: CreateAccountPayableDto) {
  const { data } = await api.post<ResourceResponse<AccountPayable>>(
    basePath,
    mapPayload(payload)
  );
  return unwrap<AccountPayable>(data as any);
...
export async function updateAccountPayable(
  id: number,
  payload: UpdateAccountPayableDto
) {
  const { data } = await api.put<ResourceResponse<AccountPayable>>(
    `${basePath}/${id}`,
    mapPayload(payload)
  );
  return unwrap<AccountPayable>(data as any);
...
export async function markAccountAsPaid(
  id: number,
  payload: { payment_method: string; payment_date: string }
) {
  const { data } = await api.patch<ResourceResponse<AccountPayable>>(
